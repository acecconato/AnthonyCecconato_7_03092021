  ...
  <section class="container">
    ...
    <Form v-if="!success" class="signup-form row" @submit="onSubmit" :validation-schema="schema">
      ...
      <div class="mb-3">
        ...
        <Field name="repeat" type="password" class="form-control" id="repeat"/>
      ...
      <div class="col-auto form-actions mt-4">
        ...
        <button type="submit" class="btn btn-primary mb-3">Créer mon compte</button>
      </div>
    </Form>

    <div v-if="message && !success">
        <p class="alert alert-danger" v-for="(msg, i) in message" :key="i">{{ msg.message }}</p>
    </div>
  ...
  data () {
    const schema = yup.object().shape({
      username: yup.string()
        ...
        .min(3, 'Doit avoir au moins 3 charactères')
      ...
      email: yup.string()
        ...
        .min(5, 'Doit avoir au moins 5 charactères')
